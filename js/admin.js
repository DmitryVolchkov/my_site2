...
(function () {
  ...
  var API_DRAFTS_URL = '/api/drafts';
  var API_DRAFT_BATCHES_URL = '/api/drafts/batches';
  var API_DRAFT_EVENTS_URL = '/api/drafts/events';
  var API_DRAFT_SOURCES_URL = '/api/drafts/sources';
  ...
  var CSV_HEADERS = [
    ...
    'attached_to', 'composite_headline', 'use_composite_headline', 'attachment_order'
  ];
  ...
  async function loadDraftsFromServer() {
    var r = await fetch(API_DRAFTS_URL, { cache: 'no-store' });
    var data = await r.json().catch(function () { return {}; });
    if (!r.ok) throw new Error(data.error || 'Не удалось загрузить черновики.');
    return { batches: data.batches || [], events: data.events || [] };
  }

  async function saveDraftBatch(item) {
    var r = await fetch(API_DRAFT_BATCHES_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(item)
    });
    var data = await r.json().catch(function () { return {}; });
    if (!r.ok) throw new Error(data.error || 'Не удалось сохранить лист.');
    return data.item;
  }

  async function deleteDraftBatchFromServer(id) {
    var r = await fetch(API_DRAFT_BATCHES_URL + '/' + encodeURIComponent(id), { method: 'DELETE' });
    var data = await r.json().catch(function () { return {}; });
    if (!r.ok) throw new Error(data.error || 'Не удалось удалить лист.');
  }

  async function saveDraftEvent(item) {
    var r = await fetch(API_DRAFT_EVENTS_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(item)
    });
    var data = await r.json().catch(function () { return {}; });
    if (!r.ok) throw new Error(data.error || 'Не удалось сохранить строку черновика.');
    return data.item;
  }

  async function deleteDraftEventFromServer(id) {
    var r = await fetch(API_DRAFT_EVENTS_URL + '/' + encodeURIComponent(id), { method: 'DELETE' });
    var data = await r.json().catch(function () { return {}; });
    if (!r.ok) throw new Error(data.error || 'Не удалось удалить строку.');
  }

  async function saveDraftSource(item) {
    var r = await fetch(API_DRAFT_SOURCES_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(item)
    });
    var data = await r.json().catch(function () { return {}; });
    if (!r.ok) throw new Error(data.error || 'Не удалось сохранить источник черновика.');
    return data.item;
  }

  async function deleteDraftSourceFromServer(id) {
    var r = await fetch(API_DRAFT_SOURCES_URL + '/' + encodeURIComponent(id), { method: 'DELETE' });
    var data = await r.json().catch(function () { return {}; });
    if (!r.ok) throw new Error(data.error || 'Не удалось удалить источник.');
  }

  async function importDraftEventOnServer(id) {
    var r = await fetch(API_DRAFT_EVENTS_URL + '/' + encodeURIComponent(id) + '/import', { method: 'POST' });
    var data = await r.json().catch(function () { return {}; });
    if (!r.ok) throw new Error(data.error || 'Не удалось импортировать черновик.');
    return data.event;
  }

  var draftSaveTimers = {};

  function findDraftEvent(id) {
    return state.draftEvents.find(function (e) { return e.id === id; });
  }

  function findDraftSource(draftEventId, sourceId) {
    var ev = findDraftEvent(draftEventId);
    if (!ev) return null;
    return (ev.sources || []).find(function (s) { return s.id === sourceId; });
  }

  function scheduleDraftEventSave(id) {
    clearTimeout(draftSaveTimers['de-' + id]);
    draftSaveTimers['de-' + id] = setTimeout(function () {
      var ev = findDraftEvent(id);
      if (!ev) return;
      var payload = {};
      Object.keys(ev).forEach(function (key) {
        if (key !== 'sources') payload[key] = ev[key];
      });
      saveDraftEvent(payload).catch(function (err) {
        setStatus($('drafts-status'), err.message || 'Не удалось сохранить строку.', 'error');
      });
    }, 400);
  }

  function scheduleDraftSourceSave(draftEventId, sourceId) {
    clearTimeout(draftSaveTimers['ds-' + sourceId]);
    draftSaveTimers['ds-' + sourceId] = setTimeout(function () {
      var source = findDraftSource(draftEventId, sourceId);
      if (!source) return;
      saveDraftSource(source).catch(function (err) {
        setStatus($('drafts-status'), err.message || 'Не удалось сохранить источник.', 'error');
      });
    }, 400);
  }

  function draftPrecisionOptions(selected) {
    var options = [
      ['', 'Авто'],
      ['day', 'День'],
      ['month', 'Месяц'],
  ...
  function draftEventsForActiveBatch() {
    return state.draftEvents
      .filter(function (e) { return e.batch_id === state.activeDraftBatchId; })
      .sort(function (a, b) { return (parseInt(a.row_order, 10) || 0) - (parseInt(b.row_order, 10) || 0); });
  }

  function renderDraftGrid() {
    var tbody = $('draft-grid-body');
    if (!tbody) return;
    ...
    rows.forEach(function (ev) {
      ...
      html += '<tr class="draft-row' + (imported ? ' is-imported' : '') + '" data-draft-id="' + escapeAttr(ev.id) + '">' +
        ...
        '<td><input type="number" class="draft-field-month" data-field="start_month" data-draft-id="' + escapeAttr(ev.id) + '" value="' + escapeAttr(ev.start_month || '') + '"' + (imported ? ' disabled' : '') + '></td>' +
        '<td><input type="number" class="draft-field-day" data-field="start_day" data-draft-id="' + escapeAttr(ev.id) + '" value="' + escapeAttr(ev.start_day || '') + '"' + (imported ? ' disabled' : '') + '></td>' +
        '<td><select data-field="start_date_precision" data-draft-id="' + escapeAttr(ev.id) + '"' + (imported ? ' disabled' : '') + '>' + draftPrecisionOptions(ev.start_date_precision) + '</select></td>' +
        '<td><input type="text" data-field="event_type" data-draft-id="' + escapeAttr(ev.id) + '" value="' + escapeAttr(ev.event_type || '') + '"' + (imported ? ' disabled' : '') + '></td>' +
        '<td><input type="text" data-field="domain" data-draft-id="' + escapeAttr(ev.id) + '" value="' + escapeAttr(ev.domain || '') + '"' + (imported ? ' disabled' : '') + '></td>' +
        '<td><input type="text" data-field="country_name" data-draft-id="' + escapeAttr(ev.id) + '" value="' + escapeAttr(ev.country_name || '') + '"' + (imported ? ' disabled' : '') + '></td>' +
        '<td>' + sourceCount + '</td>' +
        '<td><span class="draft-row-badge' + (imported ? ' is-imported' : '') + '">' + (imported ? 'Импортировано' : 'Черновик') + '</span></td>' +
        '<td class="actions">' +
        (imported ? '' : '<button type="button" class="admin-btn" data-action="import-row" data-id="' + escapeAttr(ev.id) + '">Импортировать</button>') +
        '<button type="button" class="admin-btn admin-btn-danger" data-action="delete-row" data-id="' + escapeAttr(ev.id) + '">Удалить</button>' +
        '</td>' +
        '</tr>';
      if (expanded) {
  ...
  function renderDraftSourcesPanel(ev) {
    ...
    var rowsHtml = sources.map(function (s) {
      return '<tr data-draft-source-id="' + escapeAttr(s.id) + '">' +
        '<td><input type="text" data-field="title" data-draft-event-id="' + escapeAttr(ev.id) + '" data-draft-source-id="' + escapeAttr(s.id) + '" value="' + escapeAttr(s.title || '') + '"></td>' +
        '<td><input type="text" data-field="url" data-draft-event-id="' + escapeAttr(ev.id) + '" data-draft-source-id="' + escapeAttr(s.id) + '" value="' + escapeAttr(s.url || '') + '"></td>' +
        '<td><input type="text" data-field="type" data-draft-event-id="' + escapeAttr(ev.id) + '" data-draft-source-id="' + escapeAttr(s.id) + '" value="' + escapeAttr(s.type || '') + '"></td>' +
        '<td><input type="text" data-field="author" data-draft-event-id="' + escapeAttr(ev.id) + '" data-draft-source-id="' + escapeAttr(s.id) + '" value="' + escapeAttr(s.author || '') + '"></td>' +
        '<td><input type="text" data-field="source_date" data-draft-event-id="' + escapeAttr(ev.id) + '" data-draft-source-id="' + escapeAttr(s.id) + '" value="' + escapeAttr(s.source_date || '') + '"></td>' +
        '<td><input type="text" data-field="citation" data-draft-event-id="' + escapeAttr(ev.id) + '" data-draft-source-id="' + escapeAttr(s.id) + '" value="' + escapeAttr(s.citation || '') + '"></td>' +
        '<td><textarea rows="2" data-field="evidence_quote" data-draft-event-id="' + escapeAttr(ev.id) + '" data-draft-source-id="' + escapeAttr(s.id) + '">' + escapeHtml(s.evidence_quote || '') + '</textarea></td>' +
        '<td><button type="button" class="admin-btn admin-btn-danger" data-action="delete-source" data-draft-event-id="' + escapeAttr(ev.id) + '" data-id="' + escapeAttr(s.id) + '">Удалить</button></td>' +
        '</tr>';
    }).join('');
    return '<div class="draft-sources-panel">' +
      '<table class="draft-sources-table">' +
      '<thead><tr><th>Название</th><th>URL</th><th>Тип</th><th>Автор</th><th>Дата документа</th><th>Шифр</th><th>Цитата</th><th></th></tr></thead>' +
      '<tbody>' + rowsHtml + '</tbody>' +
      '</table>' +
      '<button type="button" class="admin-btn" data-action="add-source" data-id="' + escapeAttr(ev.id) + '">+ Источник</button>' +
      '<div class="draft-event-notes">' +
      '<div class="admin-field"><label>Краткое описание</label><textarea rows="3" data-field="summary" data-draft-id="' + escapeAttr(ev.id) + '">' + escapeHtml(ev.summary || '') + '</textarea></div>' +
      '<div class="admin-field"><label>Текст</label><textarea rows="3" data-field="text" data-draft-id="' + escapeAttr(ev.id) + '">' + escapeHtml(ev.text || '') + '</textarea></div>' +
      '</div>' +
      '</div>';
  }
  ...
  function handleDraftFieldChange(target) {
    var sourceId = target.dataset.draftSourceId;
    var field = target.dataset.field;
    if (!field) return;
    var value = target.value;
    if (sourceId) {
      var draftEventId = target.dataset.draftEventId;
      var source = findDraftSource(draftEventId, sourceId);
      if (!source) return;
      source[field] = value;
      scheduleDraftSourceSave(draftEventId, sourceId);
      return;
    }
    var draftId = target.dataset.draftId;
    if (!draftId) return;
    var ev = findDraftEvent(draftId);
    if (!ev) return;
    ev[field] = value;
    scheduleDraftEventSave(draftId);
  }

  async function addDraftRow() {
    if (!state.activeDraftBatchId) return;
    var rows = draftEventsForActiveBatch();
    var nextOrder = rows.length ? Math.max.apply(null, rows.map(function (r) { return parseInt(r.row_order, 10) || 0; })) + 1 : 0;
    try {
      var saved = await saveDraftEvent({ id: '', batch_id: state.activeDraftBatchId, row_order: String(nextOrder) });
      saved.sources = [];
      state.draftEvents.push(saved);
      renderDraftGrid();
    } catch (err) {
      setStatus($('drafts-status'), err.message || 'Не удалось добавить строку.', 'error');
    }
  }

  async function addDraftSource(draftEventId) {
    try {
      var saved = await saveDraftSource({ id: '', draft_event_id: draftEventId });
      var ev = findDraftEvent(draftEventId);
      if (!ev) return;
      ev.sources = ev.sources || [];
      ev.sources.push(saved);
      renderDraftGrid();
    } catch (err) {
      setStatus($('drafts-status'), err.message || 'Не удалось добавить источник.', 'error');
    }
  }

  async function deleteDraftSourceRow(draftEventId, sourceId) {
    try {
      await deleteDraftSourceFromServer(sourceId);
      var ev = findDraftEvent(draftEventId);
      if (ev) ev.sources = (ev.sources || []).filter(function (s) { return s.id !== sourceId; });
      renderDraftGrid();
    } catch (err) {
      setStatus($('drafts-status'), err.message || 'Не удалось удалить источник.', 'error');
    }
  }

  async function deleteDraftRow(id) {
    if (!confirm('Удалить строку черновика ' + id + '?')) return;
    try {
      await deleteDraftEventFromServer(id);
      state.draftEvents = state.draftEvents.filter(function (e) { return e.id !== id; });
      renderDraftGrid();
    } catch (err) {
      setStatus($('drafts-status'), err.message || 'Не удалось удалить строку.', 'error');
    }
  }

  async function importDraftRow(id) {
    if (!confirm('Импортировать эту строку в основную БД событий?')) return;
    var status = $('drafts-status');
    try {
      await importDraftEventOnServer(id);
      var fresh = await loadDraftsFromServer();
      state.draftBatches = fresh.batches;
      state.draftEvents = fresh.events;
      renderDrafts();
      setStatus(status, 'Событие импортировано в «Наполнение» со статусом «Черновик».', 'ok');
    } catch (err) {
      setStatus(status, err.message || 'Не удалось импортировать черновик.', 'error');
    }
  }
  ...
  function toggleDraftBatchForm(show) {
    ...
    if (show) {
      form.reset();
      var titleInput = $('draft-batch-title');
      if (titleInput) titleInput.focus();
    }
  }

  async function submitDraftBatchForm(e) {
    e.preventDefault();
    var title = ($('draft-batch-title').value || '').trim();
    var targetDate = ($('draft-batch-date').value || '').trim();
    if (!title) return;
    try {
      var saved = await saveDraftBatch({ id: '', title: title, target_date: targetDate });
      state.draftBatches.push(saved);
      state.activeDraftBatchId = saved.id;
      toggleDraftBatchForm(false);
      renderDrafts();
    } catch (err) {
      setStatus($('drafts-status'), err.message || 'Не удалось создать лист.', 'error');
    }
  }

  async function deleteDraftBatch() {
    var id = state.activeDraftBatchId;
    if (!id) return;
    var batch = state.draftBatches.find(function (b) { return b.id === id; });
    if (!confirm('Удалить лист «' + (batch ? batch.title : id) + '» вместе со всеми строками и источниками?')) return;
    try {
      await deleteDraftBatchFromServer(id);
      state.draftBatches = state.draftBatches.filter(function (b) { return b.id !== id; });
      state.draftEvents = state.draftEvents.filter(function (e) { return e.batch_id !== id; });
      state.activeDraftBatchId = null;
      renderDrafts();
    } catch (err) {
      setStatus($('drafts-status'), err.message || 'Не удалось удалить лист.', 'error');
    }
  }
  ...
  function bindDrafts() {
    ...
    var deleteBatchBtn = $('btn-draft-batch-delete');
    ...
    if (batchForm) batchForm.addEventListener('submit', submitDraftBatchForm);
    if (deleteBatchBtn) deleteBatchBtn.addEventListener('click', deleteDraftBatch);
    if (newRowBtn) newRowBtn.addEventListener('click', addDraftRow);
    if (refreshBtn) {
      refreshBtn.addEventListener('click', async function () {
        try {
          ...
          state.draftEvents = fresh.events;
          renderDrafts();
          setStatus($('drafts-status'), 'Черновики обновлены.', 'ok');
        } catch (err) {
          setStatus($('drafts-status'), err.message || 'Не удалось обновить черновики.', 'error');
        }
      });
    }
    ...
    if (gridBody) {
      ...
      gridBody.addEventListener('click', function (e) {
        ...
        if (action === 'toggle-sources') {
          ...
          state.expandedDraftEventIds[id] = !state.expandedDraftEventIds[id];
          renderDraftGrid();
        } else if (action === 'import-row') {
          importDraftRow(btn.dataset.id);
        } else if (action === 'delete-row') {
          deleteDraftRow(btn.dataset.id);
        } else if (action === 'add-source') {
          addDraftSource(btn.dataset.id);
        } else if (action === 'delete-source') {
          deleteDraftSourceRow(btn.dataset.draftEventId, btn.dataset.id);
        }
      });
    }
  }
  ...
  async function initData() {
    ...
    if (canEdit()) {
      ...
      try {
        var drafts = await loadDraftsFromServer();
        state.draftBatches = drafts.batches;
        state.draftEvents = drafts.events;
      } catch (err) {
        state.draftBatches = [];
        state.draftEvents = [];
        console.warn(err);
  ...
  function clearEventForm() {
    ...
    $('event-summary').value = '';
    $('event-related').value = '';
    $('event-attached-to').value = '';
    $('event-attachment-order').value = '';
    $('event-composite-headline').value = '';
    $('event-use-composite-headline').checked = false;
    updateEventStatusSelectStyle();
    updateEventVerificationSelectStyle();
    updateEventStatusHint();
  ...
  function fillEventForm(ev) {
    ...
    $('event-attached-to').value = ev.attached_to || '';
    $('event-attachment-order').value = ev.attachment_order || '';
    $('event-composite-headline').value = ev.composite_headline || '';
    $('event-use-composite-headline').checked = ev.use_composite_headline === '1';
  ...
  function attachedChildrenCount(eventId) {
    return state.events.filter(function (ev) { return ev.attached_to === eventId; }).length;
  }

  function updateEventAttachmentHint() {
    var hint = $('event-attachment-hint');
    var attachedToEl = $('event-attached-to');
    if (!hint || !attachedToEl) return;
    var attachedTo = (attachedToEl.value || '').trim();
    if (!attachedTo) {
      hint.textContent = 'Если заполнено — это событие показывается одной точкой шкалы вместе с основным. Дата должна совпадать с основным событием, иначе сохранить не получится.';
      hint.className = 'admin-hint field-full';
      return;
    }
    ...
    var sameDate = (target.start_year || '') === (form.start_year || '')
      && (target.start_month || '') === (form.start_month || '')
    ...
    hint.className = 'admin-hint field-full is-success';
  }
  ...
  function renderEventPreview() {
    ...
    if (notice) {
      ...
      if (form.attached_to) {
        notice.textContent += ' Показывается одной точкой шкалы вместе с ' + form.attached_to + ', заголовок и текст этой формы — как вторичные в объединённой карточке.';
      } else {
  ...
  function readEventForm() {
    return {
      ...
      attached_to: ($('event-attached-to').value || '').trim(),
      attachment_order: ($('event-attachment-order').value || '').trim(),
      composite_headline: ($('event-composite-headline').value || '').trim(),
      use_composite_headline: $('event-use-composite-headline').checked ? '1' : '0',
